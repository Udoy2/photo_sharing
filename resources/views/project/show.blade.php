...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <div class="float-right">
            <a href="{{route('folders.create_new_folder_show',$project->id)}}"  class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Add Folder</a>
        </div>
    ...
    <div class="card-body">
        ...
        @isset($project->folders )
                ...
                @foreach ($project->folders as $folder)
                    <div class="text-center" style="width:100px">
                        <a href="{{route('folders.show', $folder->id)}}">
                            ...
                            <div> {{$folder->name}}</div><br>
                            <form action="{{action('FolderController@destroy', $folder->id)}}" id="folder_delete_form" method="POST">

                                @csrf
                                @method('DELETE')
            
                                <a onclick="document.getElementById('folder_delete_form').submit();" ><i class="fa fa-trash text-danger"></i></a>
            ...
            @else
            <p>No folder created <a href="{{route('folders.create_new_folder_show',$project->id)}}" class="btn btn-sm btn-success">Add Folder</a></p>
            @endif
        @endisset
