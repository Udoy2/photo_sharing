...
@section('css')
   <style>
            .shade1::before{
               ...
               width: 100%;
               height: 100%;
               background: linear-gradient(to top, rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0));
      ...
      <div class="filter p-3 container-fluid">
         <div class="row">
            <div class="form-group form-inline w-100 pl-5 mb-0">
               <select id="select2" tabindex="-1"  class="select2-hidden-accessible form-control w-16" aria-hidden="true"  onchange="selectChangeProject()">
                  <option value="all" >All Photos</option>
                  @isset($project->folders)
                    @foreach ($project->folders as $folder)
                        <option value="client_folder_{{$folder->id}}" >{{$folder->name}}</option>
                    @endforeach
                  @endisset
               </select>
         ...
         <div class="container-fluid">
            <div class="demo-gallery">
               <div id="lightgallery" class=" masonry-grid">
                  ...
                  <figure class="item client_folder_{{$folder->id}} client_area_all_photo" data-responsive="{{asset('public/uploads/'.Str::lower($folder->project->name).'/'.Str::lower($folder->name).'/'.$photo->photo)}} 375, 
                     ...
                     <img class="img-fluid"  src="{{asset('public/uploads/'.Str::lower($folder->project->name).'/'.Str::lower($folder->name).'/'.$photo->photo)}}" alt="Thumb-1">
                     </a>
      ...
      <div class="row">
         <div class="col-md-4 m-auto text-center">
            ...
            <div class="form-group mt-4">
               <input type="text" name="name" class="form-control  rounded-0" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{ old('name') }}" placeholder="Enter your name">
               

            </div>
            <div class="form-group">
               <input type="email" name="email" class="form-control rounded-0" id="exampleInputEmail1" aria-describedby="emailHelp" value="{{ old('email') }}" placeholder="Enter email">
            </div>
            <div class="form-group">
               <input type="password" name="password" class="form-control rounded-0" id="exampleInputPassword1" placeholder="PIN"><br>
...
<script>
     function selectChangeProject() {
         var client_area_project_class = document.getElementById("select2").value;
         if(client_area_project_class == 'all'){
            $('.client_area_all_photo').fadeIn("slow");
         }else{
            $('.client_area_all_photo').fadeOut("slow");
            $('.'+client_area_project_class).fadeIn("slow");
         }
